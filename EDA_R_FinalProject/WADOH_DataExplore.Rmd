...
prop.table(with(vacc,table(school_type, reported,school_year == "2011")),1)[,2:3,2]*100
prop.table(with(vacc,table(school_type, reported,school_year == "2012")),1)[,2:3,2]*100
prop.table(with(vacc,table(school_type, reported,school_year == "2013")),1)[,2:3,2]*100
...
vaccReported$nonmedical_exempt <- sum(vacc$religious_exempt,vacc$personal_exempt)

gg <- vaccReported[is.na(vaccReported$religious_exempt),]
is.na(vaccReported$personal_exempt)
#there are Na's where the value should be 0 or null (example, 1 exemption, that exemption is personal, therefore medical and religious are Na, ask coaches how to handle this)
```
...
#non-medical exempt
ggplot(aes(x = non), data = vaccReported) + 
  geom_histogram(binwidth = .1,  color = 'black',fill = '#F79420') +
  facet_wrap(~school_year) +
  scale_x_log10() +
