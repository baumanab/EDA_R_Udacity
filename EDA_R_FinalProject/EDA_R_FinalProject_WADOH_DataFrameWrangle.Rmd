...
doh_k12_2011<- read.delim('data/2011.txt', quote="" , stringsAsFactors=FALSE)

#WA DOH SY 2012-2013 K-12
doh_k12_2012<- read.delim('data/2012.txt', quote="" , stringsAsFactors=FALSE)

#WA doh SY 2013-2014 K-12
doh_k12_2013<- read.delim('data/2013.txt', quote="" , stringsAsFactors=FALSE)
...
#WA OSIP 1
WA_OSIP_1<- read.delim('data/school_lists/482_SDF_bldg.txt', quote="" , stringsAsFactors=FALSE)

#WA OSIP 2
WA_OSIP_2<- read.delim('data/school_lists/483_SDF_bldg.txt', quote="" , stringsAsFactors=FALSE)

#WA OSIP 3
WA_OSIP_3<- read.delim('data/school_lists/491_SDF_ApprovedPrivateSchoolsList.txt', quote="" , stringsAsFactors=FALSE)

#WA OSIP 4
WA_OSIP_4<- read.delim('data/school_lists/492_SDF_ApprovedPrivateSchoolsList.txt', quote="" , stringsAsFactors=FALSE)
...
WA_DOH_combined$school_year <- as.factor(WA_DOH_combined$school_year)
WA_DOH_combined$reported <- as.factor(WA_DOH_combined$reported)
```

```{r cleanup reported field}
#reported field has 3 levels, it should have 2, yes or no
WA_DOH_combined[WA_DOH_combined$reported == "",]

#These rows have no real information in them, removing from combined DF
WA_DOH_combined <- WA_DOH_combined[WA_DOH_combined$reported != "",]

```
```{r cleanup school type}
#unique values for school type
unique(WA_DOH_combined$school_type)
#convert type (currently factor) to character
WA_DOH_combined$school_type <- as.character(WA_DOH_combined$school_type)

#split on space and take first word
WA_DOH_combined$school_type <- sapply(strsplit(WA_DOH_combined$school_type,split = " "),"[",1)

#convert back to factor
WA_DOH_combined$school_type <- as.factor(WA_DOH_combined$school_type)

```
###Fill Na for exemption types###
Some values for exemption by type that are reported and have total exemptions are errouneously listed as Na.  Many or all of these should be 0.  For example if the total exemptions for an observation = 1, and there is a value of 1 assigned to personal exemptions, then the other exemption types for that observation should be 0, not Na.

```{r reconciling Na for exemption types}
#find the erroneous Na
ex_test <- subset(WA_DOH_combined,reported == "Yes")
ex_test_mpr <-subset(ex_test,is.na(religious_exempt) | is.na(medical_exempt) | is.na(personal_exempt))

#all 37 records are reported and have exemption type counts reconciled to total, so Na should be 0
ex_test_mpr$religious_exempt[is.na(ex_test_mpr$religious_exempt)] <- 0
ex_test_mpr$medical_exempt[is.na(ex_test_mpr$medical_exempt)] <- 0

#assign rows to be removed
remove <- row.names(subset(WA_DOH_combined, (reported == "Yes") & (is.na(medical_exempt) | is.na(religious_exempt))))

#remove rows
WA_DOH_combined <- WA_DOH_combined[!rownames(WA_DOH_combined) %in% remove, ]

#recombine with corrected rows
WA_DOH_combined <- rbind(WA_DOH_combined,ex_test_mpr)

```


```{r save and export WA DOH combined DF}
write.table(WA_DOH_combined, "data/WA_DOH_Combined.txt", sep ="\t")

saveRDS(WA_DOH_combined,"data/WA_DOH_Combined.rds")

```


```{r for data review}
#what I started with after conversion from .xlsx
str(read.delim('data/2011.txt', quote="" , stringsAsFactors=FALSE))
summary(read.delim('data/2011.txt', quote="" , stringsAsFactors=FALSE))

str(read.delim('data/2012.txt', quote="" , stringsAsFactors=FALSE))
summary(read.delim('data/2012.txt', quote="" , stringsAsFactors=FALSE))

str(read.delim('data/2013.txt', quote="" , stringsAsFactors=FALSE))
summary(read.delim('data/2013.txt', quote="" , stringsAsFactors=FALSE))
