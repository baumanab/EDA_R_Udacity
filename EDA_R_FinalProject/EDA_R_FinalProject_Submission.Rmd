...
ggplot(data= county_group, aes(y=mean_percent_exempt_MMR, x=reorder(school_county, students))) +
  geom_bar(position="dodge",stat="identity", color = "red", fill = "orange") + 
  coord_flip() +
  xlab("County")
...
ggplot(data= melt_county_group, aes(y=mean_percent_exemption_type, x=reorder(school_county, students), fill = exemption_type)) +
  ...
  theme(legend.position = "top", legend.background = element_rect(color = "black", 
    fill = "grey90", size = 1, linetype = "solid"), legend.direction = "horizontal", legend.title=element_blank())
...
#order by students enrolled, so essentially the student population of each county
