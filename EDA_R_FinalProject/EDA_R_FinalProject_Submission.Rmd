...
ggplot(aes(x = enrolled), data = vaccReported) + 
  geom_histogram(binwidth = log(1.01),  color = 'black',fill = '#F79420') +
  scale_x_log10(breaks = brks) +
  scale_y_continuous(breaks = seq(0,200,10))
...
ggplot(aes(x = total_exemptions), data = vaccReported) + 
  geom_histogram(binwidth = log(1.25),  color = 'black',fill = 'red') +
  scale_x_log10(breaks = brks) +
...
t1 = ggplot(data = vaccReported, aes(x = percent_nonmedexempt, y = ..density..)) +
  geom_freqpoly(aes(color = school_type)) +
  scale_x_log10()

names(vaccReported)

test <- gather(vaccReported_NLT_10_NGT_2500,exemption_type,exemptions,medical_exempt, nonmedical_exempt)

#add percent by exemption type
test$percent_exemption_type <- round((test$exemptions/test$total_exemptions)*100,digits = 2)

ggplot(data = test, aes(x = percent_exemption_type, y = ..density..)) +
  geom_freqpoly(binwidth = log(1.25),aes(color = exemption_type)) +
  facet_wrap(school_type ~ school_year) +
  scale_x_log10()


ggplot(data = subset(test, exemption_type == 'medical_exempt'), aes(x = percent_exemption_type, y = ..density..)) +
  geom_freqpoly(binwidth = log(1.25),aes(color = school_type)) +
  facet_wrap(~school_year) +
  scale_x_log10()

ggplot(data = subset(test, exemption_type == 'nonmedical_exempt'), aes(x = percent_exemption_type, y = ..density..)) +
  geom_freqpoly(binwidth = log(1.25),aes(color = school_type)) +
  facet_wrap(~school_year) +
  scale_x_log10()

ggplot(data = subset(test, exemption_type == 'nonmedical_exempt'), aes(x = percent_exemption_type, y = ..count../sum(..count..))) +
...
##top 20 percent exempt
top20_allyears <- arrange(vaccReported_NLT_10_NGT_2500, desc(percent_exempt))[1:20,c("school_year","school_name", "school_county", "school_city","enrolled", "total_exemptions", "percent_exempt", "percent_medical_exempt","percent_nonmedexempt","percent_exempt_MMR")]
