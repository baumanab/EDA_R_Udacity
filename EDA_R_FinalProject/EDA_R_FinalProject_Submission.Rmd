...
ggplot(aes(x = enrolled), data = vaccReported) + 
  ...
  scale_x_continuous(limits = c(0,5100), breaks = seq(0, 5100, 100))
...
ggplot(aes(x = total_exemptions), data = vaccReported) + 
  ...
  scale_x_continuous(limits = c(0,450), breaks = seq(0, 450, 10)) 
...
ggplot(aes(x = total_exemptions), data = vaccReported) + 
  geom_histogram(binwidth = .11,  color = 'black',fill = '#F79420') +
  scale_x_log10()
...
ggplot(aes(x = percent_medical_exempt, y = ..count../sum(..count..)), data = vaccReported) +
  geom_freqpoly(binwidth = 1, aes(color = school_year))
