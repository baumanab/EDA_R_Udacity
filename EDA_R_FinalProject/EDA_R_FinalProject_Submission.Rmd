...
Splitting by quartile but using just the mean exemption rate (not from each type).

```{r echo=FALSE}


names(melt_county_group)

names(county_group)

diagnostic <- county_group[county_group$school_county == "FERRY",]

diagnostic2 <-melt_county_group[melt_county_group$school_county == "FERRY",]


county_group$school_county
p9 = ggplot(data= data_4, aes(y=mean_percent_exempt/2, x= reorder(school_county, students))) +
  geom_bar(position="stack",stat="identity", aes(color = "black", fill = "blue")) + 
  coord_flip() +
  theme(legend.position = "none",legend.title=element_blank(),axis.title.x = element_blank()) +
  geom_text(aes(label = shortlabel_4, y = max(data_4$mean_percent_exemption_type)), fontface = "bold") +
  geom_rect(aes(xmin=0, xmax=10.5, ymin=8, ymax= max(data_4$mean_percent_exemption_type)), fill = "red", alpha=0.02) +
...
p10 = ggplot(data= data_3, aes(y=mean_percent_exemption_type, x= reorder(school_county, students), fill = exemption_type)) +
  geom_bar(position="stack",stat="identity") + 
  coord_flip() +
  theme(legend.position = "none",legend.title=element_blank(),axis.title.x = element_blank()) +
  geom_text(aes(label = shortlabel_3, y = max(data_3$mean_percent_exemption_type)), fontface = "bold") +
...
p11 = ggplot(data= data_2, aes(y=mean_percent_exemption_type, x= reorder(school_county, students), fill = exemption_type)) +
  geom_bar(position="stack",stat="identity") + 
  coord_flip() +
  theme(legend.position = "none",legend.title=element_blank(),axis.title.x = element_blank()) +
  geom_text(aes(label = shortlabel_2, y = max(data_2$mean_percent_exemption_type)), fontface = "bold") +
...
p12 = ggplot(data= data_1, aes(y=mean_percent_exemption_type, x= reorder(school_county, students), fill = exemption_type)) +
  geom_bar(position="stack",stat="identity") + 
  coord_flip() +
  theme(legend.position = "none",legend.title=element_blank()) +
  geom_text(aes(label = shortlabel_1, y = max(data_1$mean_percent_exemption_type)), fontface = "bold") +
...
ggplot(data= county_group, aes(y=mean_percent_exempt, x=reorder(school_county, students), fill = mean_percent_exempt_MMR)) +
  ...
  xlab("County") +
  theme(legend.position = c(.7,.8), legend.background = element_rect(color = "black", 
    fill = "grey90", size = .5, linetype = "solid"), legend.direction = "vertical", legend.title = element_text("mean % exempt MMR"))
