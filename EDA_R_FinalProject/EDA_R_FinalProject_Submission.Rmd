...
ggplot(data= county_group, aes(y=mean_percent_exempt_MMR, x=reorder(school_county, students))) +
  geom_bar(position="dodge",stat="identity", color = "green", fill = "blue") + 
  coord_flip() +
  xlab("County")
...
fn_labels <- function(quartile){
  return(unique(melt_county_group$labels[melt_county_group$mean_percent_exempt_quartile == quartile]))
}
...
ggplot(data= subset(melt_county_group, mean_percent_exempt_quartile == 4), aes(y=mean_percent_exemption_type, x= reorder(school_county, students), fill = exemption_type)) +
  geom_bar(position="stack",stat="identity") + 
  coord_flip() +
  geom_text(aes(label = labels, y = 23)) +
  theme(legend.position = "top",legend.title=element_blank()) +
