...
```{r Test code on WA DOH codeframes}
#Test function concept
WA_DOH_codeframe$school_zip[WA_DOH_codeframe$school_code == "4178"]

WA_DOH_codeframe$school_zip[WA_DOH_codeframe$school_code == doh_k12_2012$school_code[3]]
...
doh_k12_2012$school_zip <- apply(doh_k12_2012[,c("school_code"), drop = F],1, function(x) {WA_DOH_codeframe$school_zip[WA_DOH_codeframe$school_code == x]})
```
...
#used sapply or mapply here, but instead chose drop = False to work on a single
#vector (column) because I may re-use this code on multiple columns later.
doh_k12_2012$school_zip <- apply(doh_k12_2012[,c("school_code"), drop = F],1, function(x) {final_codeframe$school_zip[final_codeframe$school_code == x]})

#collapse lists
doh_k12_2012$school_zip <- unique(doh_k12_2012$school_zip)

#handle character(0)
duplicated(final_codeframe$school_code)
#insert school_city
doh_k12_2012$school_city <- apply(doh_k12_2012[,c("school_code"), drop = F],1, function(x) {final_codeframe$school_city[final_codeframe$school_code == x]})

#subset final codeframe to eliminate district code placeholder
final_codeframe_dcodefilter<- final_codeframe[final_codeframe$district_code != 8675309,]

#filter final codeframe for unique school code values
final_codeframe_dcodefilter <- final_codeframe_dcodefilter[!duplicated(final_codeframe_dcodefilter$school_code),]

#insert district code 2012
doh_k12_2012$district_code <- apply(doh_k12_2012[,c("school_code"), drop = F],1, function(x) {final_codeframe_dcodefilter$district_code[final_codeframe_dcodefilter$school_code == x]})

#insert district code 2013
doh_k12_2013$district_code <- apply(doh_k12_2013[,c("school_code"), drop = F],1, function(x) {final_codeframe_dcodefilter$district_code[final_codeframe_dcodefilter$school_code == x]})
