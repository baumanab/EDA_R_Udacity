...
cols <- c("school_code", "school_city","school_zip")

OSIP_codeframe<- rbind(WA_OSIP_1[cols],WA_OSIP_2[cols],WA_OSIP_3[cols],WA_OSIP_4[cols])

#clean city field (web addresses and empty)
OSIP_codeframe <- OSIP_codeframe[grepl("www",OSIP_codeframe$school_city) == F,]
OSIP_codeframe <- OSIP_codeframe[grepl(" ",OSIP_codeframe$school_city) == F,]


#omit NA
OSIP_codeframe <- na.omit(OSIP_codeframe)



#srip zipcode at hypehn ("-"), extract first five digits
sapply(strsplit(OSIP_codeframe$school_zip,split = "-"),"[",1)
```
...
####School Code Audit####
Going to try subset on this.  I will subset 2012 and 2013 on those school codes != to 2011, if any.  Using the %in% operator and subsetting on False (not in year). 
```{r School Code Audit}
