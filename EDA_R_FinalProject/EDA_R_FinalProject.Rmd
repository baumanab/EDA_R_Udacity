...
library(GGally)
library(reshape2)
#library(acs) #to access census data, commented out because masks base apply
...
rm(WA_OSIP_1, WA_OSIP_2,WA_OSIP_3,WA_OSIP_4,WA_DOH_codeframe,OSIP_codeframe_private,OSIP_codeframe_private_unique,OSIP_codeframe_public,OSIP_codeframe_public_unique,codeframe_2011,codeframe_2013,colnames_2011,colnames_2012,colnames_2013,cols_private,cols_public)

rm(doh_k12_2012_in,doh_k12_2012_out,maxdiff,reported_doh_k12_2011,reported_measles,reported_mumps,reported_rubella)

rm(select_colnames_2011,select_colnames_2013)
...
WA_DOH_tidy$percent_exempt_DTTd <- round((WA_DOH_tidy$exempt_DTTd/WA_DOH_tidy$enrolled)*100,digits = 2)

WA_DOH_tidy$percent_exempt_pertussis <- round((WA_DOH_tidy$exempt_pertussis/WA_DOH_tidy$enrolled)*100,digits = 2)

WA_DOH_tidy$percent_exempt_polio <- round((WA_DOH_tidy$exempt_polio/WA_DOH_tidy$enrolled)*100,digits = 2)

WA_DOH_tidy$percent_exempt_MMR <- round((WA_DOH_tidy$exempt_MMR/WA_DOH_tidy$enrolled)*100,digits = 2)

WA_DOH_tidy$percent_exempt_hepB <- round((WA_DOH_tidy$exempt_hepB/WA_DOH_tidy$enrolled)*100,digits = 2)

WA_DOH_tidy$percent_exempt_varicella <- round((WA_DOH_tidy$exempt_varicella/WA_DOH_tidy$enrolled)*100,digits = 2)

WA_DOH_tidy$percent_exempt <- round((WA_DOH_tidy$exempt/WA_DOH_tidy$enrolled)*100,digits = 2)

WA_DOH_tidy$percent_exemption_type <- round((WA_DOH_tidy$exemptions/WA_DOH_tidy$enrolled)*100,digits = 2)
...
WA_DOH_tidy$school_type <- as.factor(WA_DOH_tidy$school_type)
WA_DOH_tidy$year <- as.factor(WA_DOH_tidy$year)
WA_DOH_tidy$reported <- as.factor(WA_DOH_tidy$reported)
```
